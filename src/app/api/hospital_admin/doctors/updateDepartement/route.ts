...
export async function PATCH(request: NextRequest) {
    try {
        ...
        if (!doctor || doctor.hospitalId !== hospital.id) {
            return NextResponse.json({ error: "Doctor not found in this hospital" }, { status: 404 });
        }
        ...
        if (departmentId) {
            ...
            if (!dept || dept.hospitalId !== hospital.id) {
                return NextResponse.json({ error: "Department not found in this hospital" }, { status: 404 });
            }
        }
