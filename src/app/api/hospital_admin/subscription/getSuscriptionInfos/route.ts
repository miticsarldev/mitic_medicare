  ...
  try {
    ...
    return NextResponse.json({
      ...
      canAddMoreDoctors:
        ...
        (maxDoctors === Infinity || currentDoctors < maxDoctors),
      remainingSlots:
        maxDoctors === Infinity ? "Illimité" : maxDoctors - currentDoctors,
    });
  } catch (error) {
    ...
    return NextResponse.json(
      {
        error:
          "Erreur serveur : Impossible de récupérer les informations d'abonnement",
      },
      { status: 500 }
    );
  }
