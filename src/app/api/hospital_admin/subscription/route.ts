...
export async function GET() {
    ...
    if (!session?.user?.id || session.user.role !== "HOSPITAL_ADMIN") {
        return NextResponse.json(
            { error: "Accès non autorisé. Seuls les administrateurs d'hôpital peuvent accéder à ces informations." },
            { status: 401 }
        );
    }
    ...
    try {
        ...
        if (!hospitalData?.subscription) {
            return NextResponse.json(
                { 
                    ...
                    suggestion: "Veuillez souscrire à un abonnement pour accéder aux fonctionnalités complètes"
                },
                { status: 404 }
            );
        }
        ...
        const maxDoctors = DOCTOR_QUOTAS[subscription.plan] === Infinity 
            ? "Illimité" 
            : DOCTOR_QUOTAS[subscription.plan];
        
        const remainingSlots = DOCTOR_QUOTAS[subscription.plan] === Infinity
            ? "Illimité"
            : DOCTOR_QUOTAS[subscription.plan] - _count.doctors;

        const canAddMoreDoctors = (DOCTOR_QUOTAS[subscription.plan] === Infinity || _count.doctors < DOCTOR_QUOTAS[subscription.plan]);
        ...
        return NextResponse.json(
            { 
                error: "Une erreur est survenue lors de la récupération de vos informations d'abonnement",
