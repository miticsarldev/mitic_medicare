...
export async function POST(req: NextRequest) {
  ...
  const tokenRes = await fetch('https://api.orange.com/oauth/v2/token', {
    method: 'POST',
    headers: {
  ...
  if (!paymentData?.payment_url) {
    return NextResponse.json({ error: 'Erreur de création du paiement', details: paymentData }, { status: 500 });
  }
