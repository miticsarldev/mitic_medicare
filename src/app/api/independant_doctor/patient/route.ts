...
export async function GET() {
  ...
  const age = patient.user?.dateOfBirth
    ? new Date().getFullYear() - new Date(patient.user.dateOfBirth).getFullYear()
    : "Inconnu";

  return {
    id: appointment.id,
    patientId: patient.id, // Ajouté ici pour dédupliquer
...
// Dédupliquer les patients par leur patientId
const uniquePatientsMap = new Map();
formattedAppointments.forEach((appointment) => {
  if (!uniquePatientsMap.has(appointment.patientId)) {
    uniquePatientsMap.set(appointment.patientId, appointment);
  }
});

return NextResponse.json(Array.from(uniquePatientsMap.values()), { status: 200 });
