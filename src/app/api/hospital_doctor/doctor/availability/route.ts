    ...
    if (isRecurring) {
      if (!days || days.length === 0) {
        return NextResponse.json(
          {
            message:
              "Veuillez sélectionner au moins un jour pour les créneaux récurrents",
          },
          { status: 400 }
        );
      }
      ...
      if (doctorAvailabilities.length !== days.length) {
        return NextResponse.json(
          {
            message:
              "Certains jours sélectionnés ne font pas partie de vos disponibilités",
          },
          { status: 400 }
        );
      }
      ...
      await Promise.all(
        days.map(async (dayOfWeek: number) => {
      ...
      if (!specificDates || specificDates.length === 0) {
        return NextResponse.json(
          {
            message:
              "Veuillez sélectionner au moins une date pour les créneaux spécifiques",
          },
          { status: 400 }
        );
      }
      ...
      const dateDays = specificDates.map((dateStr: string) =>
      ...
      if (doctorAvailabilities.length !== dateDays.length) {
        return NextResponse.json(
          {
            message:
              "Certaines dates sélectionnées ne font pas partie de vos jours de disponibilité",
          },
          { status: 400 }
        );
      }
      ...
      await Promise.all(
        specificDates.map(async (dateStr: string) => {
