...
export default function PatientsListPage() {
  ...
  const fetchList = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/independant_doctor/patients?${qs}`, {
        cache: "no-store",
      });
  ...
  const fetchDetail = useCallback(async (id: string) => {
    ...
    setOpen(true);
    try {
      const res = await fetch(`/api/independant_doctor/patient/${id}`, {
        cache: "no-store",
      });
      if (!res.ok) throw new Error("Chargement des détails impossible");
      const data = await res.json();

      // Transform API payload to PatientDetail shape expected by UI
      const medicalHistories = (data.medicalHistories ?? []).map((h: any) => ({
        id: h.id,
        title: h.title,
        condition: h.condition,
        status: h.status,
        diagnosedDate: h.diagnosedDate ?? null,
        details: h.details ?? null,
        createdAt: h.diagnosedDate ?? new Date().toISOString(),
      }));

      const medicalRecords = (data.appointments ?? [])
        .filter((a: any) => a.medicalRecord)
        .map((a: any) => ({
          id: a.medicalRecord.id,
          createdAt: a.scheduledAt,
          diagnosis: a.medicalRecord.diagnosis,
          treatment: a.medicalRecord.treatment ?? null,
          followUpNeeded: false,
          followUpDate: null,
          doctorName: a.doctor?.user?.name ?? "",
        }));

      const vitalSigns = (data.vitalSigns ?? []).map((v: any) => ({
        id: v.id,
      ...
      const detailPayload: PatientDetail = {
        patient: {
          id: data.id,
  ...
  async function deleteHistory(id: string) {
    try {
      const res = await fetch(
        `/api/independant_doctor/patients/medical_histories/${id}`,
        { method: "DELETE" }
      );
  ...
  const fetchAvailableDates = useCallback(async () => {
    try {
      ...
      const res = await fetch(
        "/api/independant_doctor/patients/appointments/available-dates"
      );
                            ...
                            <div className="flex flex-wrap gap-2 items-center">
                              ...
                              <Button
                                ...
                                onClick={async () => {
                                  try {
                                    const res = await fetch(
                                      `/api/independant_doctor/patients/vital_signs/${v.id}`,
                                      { method: "DELETE" }
                                    );
                                    if (!res.ok)
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  const url = mhEditing.id
                    ? `/api/independant_doctor/patients/medical_histories/${mhEditing.id}`
                    : `/api/independant_doctor/patients/medical_histories`;
            ...
            <Button
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  const res = await fetch(
                    `/api/independant_doctor/patient/${detail.patient.id}/vital-signs`,
                    {
                      method: "POST",
                      headers: { "Content-Type": "application/json" },
                      body: JSON.stringify(payload),
                  ...
                  onSelect={async (date) => {
                    ...
                    if (date) {
                      ...
                      try {
                        ...
                        const res = await fetch(
                          `/api/independant_doctor/patients/appointments/available?date=${dateStr}`,
                          { cache: "no-store" }
                        );
                        const json = await res.json();
            ...
            <Button
              onClick={async () => {
                ...
                try {
                  ...
                  const res = await fetch(
                    `/api/independant_doctor/patients/appointments`,
                    {
                      method: "POST",
                      headers: { "Content-Type": "application/json" },
                      body: JSON.stringify({
