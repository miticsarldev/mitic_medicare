...
export default function PatientMedicalRecord() {
  ...
  const getBloodTypeInfo = (bloodType: string) => {
    ...
    return (
      bloodTypeMap[bloodType] || { display: bloodType, variant: "secondary" }
    );
  };
  ...
  const handleEditHistory = (
    history: PatientDetails["medicalHistories"][0]
  ) => {
    ...
    setMedicalHistoryForm({
      ...
      diagnosedDate: history.diagnosedDate
        ? new Date(history.diagnosedDate).toISOString().split("T")[0]
        : "",
    });
    setIsModalOpen(true);
  };
  ...
  const handleMedicalHistorySubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        await fetchPatientData();
        if (
          patient?.medicalHistories?.find((h) => h.id === editingHistory?.id)
            ?.title !== medicalHistoryForm.title
        ) {
          toast({
            title: "Succès",
          ...
          {completedAppointments.length > 0 && (
            <div className="flex items-center gap-2">
              ...
              <p className="text-sm text-gray-900 dark:text-gray-100">
                ...
                {formatDate(completedAppointments[0].scheduledAt)} avec{" "}
              ...
              {patient.medicalHistories?.length > 0 ? (
                <ul className="space-y-3">
                  {patient.medicalHistories.map((history) => (
                    <li
                      key={history.id}
                      className="flex justify-between items-start gap-4 p-3 border rounded-md dark:border-gray-700"
                    >
          ...
          <form onSubmit={handleVitalSignsSubmit} className="space-y-4">
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button type="submit" disabled={isUpdatingVitals}>
                {isUpdatingVitals ? <Loader className="mr-2 h-4 w-4" /> : null}
                Enregistrer
              </Button>
            </div>
          </form>
          ...
          {recordData ? (
            <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className="p-4 bg-muted/50 rounded-lg">
                    ...
                    {recordData.followUpDate && (
                      <p className="mt-2">
                        Date de suivi:{" "}
                        {new Date(recordData.followUpDate).toLocaleDateString()}
                      </p>
                    )}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="p-4 bg-muted/50 rounded-lg">
                    <p>
                      ...
                      <Badge
                        variant={
                          getBloodTypeInfo(recordData.patient.bloodType).variant
                        }
                      >
              ...
              {recordData.prescriptions?.length > 0 && (
                <div>
                  <h3 className="font-medium mb-2">
                    Prescriptions ({recordData.prescriptions.length})
                  </h3>
                  <div className="space-y-2">
      ...
      <AlertDialog open={isDeleteModalOpen} onOpenChange={setIsDeleteModalOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Cette action supprimera définitivement cet antécédent médical.
          ...
          <AlertDialogFooter>
            ...
            >
              {isDeletingHistory ? <Loader className="mr-2 h-4 w-4" /> : null}
              Supprimer
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
