      ...
      <div className="mt-6 space-y-4">
        {appointmentsByDate[selectedDate]?.length > 0 ? (
          appointmentsByDate[selectedDate].map((appointment) => (
            <div
              key={appointment.id}
              className="p-4 bg-white rounded-lg shadow-md flex justify-between items-center border-l-4 border-gray-300"
            >
              <div>
                <h3 className="text-lg font-semibold text-gray-700">{appointment.patient}</h3>
          ...
          <p className="text-gray-500 text-center">Aucun rendez-vous prévu ce jour-là.</p>
        )}
      </div>
    </div>
  );
