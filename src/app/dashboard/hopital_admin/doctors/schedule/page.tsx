...
import { format, parseISO, getHours, startOfWeek, endOfWeek, addWeeks, subWeeks, isSameWeek } from "date-fns";
import { fr } from "date-fns/locale/fr";
import {
    Select, // Keeping for import, but its specific usage for selecting weeks is removed from this component.
    SelectContent,
    SelectItem,
    SelectTrigger,
    SelectValue
...
import { CalendarClock, User, Stethoscope, Info, ChevronLeft, ChevronRight, Loader2 } from "lucide-react";
...
export default function WeeklySchedule() {
    ...
    const fetchSchedules = useCallback(async () => {
        ...
        try {
            ...
            setSchedules(data);
            // The currentWeek state already handles the initial week,
            // so no need to set a specific selectedWeek from fetched data.
        } catch (err) {
    ...
    const filteredSchedules = useMemo(() => {
        // Dynamically create the week key from the currentWeek Date object.
        // This ensures the filter always matches the week displayed by navigation.
        const weekKey = format(currentWeek, "yyyy-'S'ww", { locale: fr });
        
        return schedules
            .map(doc => ({
                ...doc,
                // Access appointments for the computed weekKey. If no appointments for this week, default to empty array.
                schedule: doc.weeklyAppointments[weekKey] || []
            }))
            // Filter by selected doctor (if any)
            .filter(doc => selectedDoctor === null || doc.id === selectedDoctor)
            .map(doc => ({
                ...doc,
                // Filter appointments within the current week by selected day (if any)
                schedule: doc.schedule.filter(apt => !selectedDay || apt.day === selectedDay)
            }));
    }, [schedules, currentWeek, selectedDoctor, selectedDay]); // Recalculates when these dependencies change
    ...
    const translateStatus = (status: string) => {
        switch (status) {
            ...
            case "CANCELED": return "Annulé"; // Corrected typo here
        ...
        <div className="p-4 space-y-4">
                            ...
                            <Tooltip>
                                <TooltipTrigger asChild>
                                    <Button 
                                        ...
                                        className="px-2"
                                    >
                                        {isChangingWeek ? <Loader2 className="h-4 w-4 animate-spin" /> : <ChevronLeft className="h-4 w-4" />}
                                    </Button>
                                </TooltipTrigger>
                            ...
                            <Tooltip>
                                <TooltipTrigger asChild>
                                    <Button 
                                        ...
                                        className="px-2"
                                    >
                                        {isChangingWeek ? <Loader2 className="h-4 w-4 animate-spin" /> : <ChevronRight className="h-4 w-4" />}
                                    </Button>
                                </TooltipTrigger>
                        ...
                        >
                            <TabsList className="grid grid-cols-4 sm:grid-cols-8 w-full">
                                ...
                                {DAYS_OF_WEEK.map(day => (
                                    ...
                                    >
                                        {day.substring(0, 3)} {/* Display first 3 letters of the day */}
                                    </TabsTrigger>
                                ))}
                            </TabsList>
                        </Tabs>
            ...
            {loading ? (
                ...
                <div className="flex flex-col items-center justify-center p-8 border rounded-lg">
                    <p className="text-muted-foreground mb-2">Aucun rendez-vous trouvé pour cette semaine et ces filtres.</p>
                    <Button
                        variant="outline"
                        size="sm"
                        onClick={() => {
                                    ...
                                    {visibleDays.map(day => {
                                        ...
                                        const appointmentsForCell = filteredSchedules.flatMap(doc =>
                                            doc.schedule.filter(apt => {
                                                ...
                                                const aptWeekKey = format(aptDate, "yyyy-'S'ww", { locale: fr });
                                                const currentDisplayWeekKey = format(currentWeek, "yyyy-'S'ww", { locale: fr });

                                                return aptWeekKey === currentDisplayWeekKey &&
                                                       format(aptDate, "EEEE", { locale: fr }).toLowerCase() === day &&
                                                        ...
                                                        <div
                                                            ...
                                                            className={`
                                                                ...
                                                                ${apt.status.toLowerCase() === 'confirmed'
                                                                    ...
                                                                    : apt.status.toLowerCase() === 'canceled' // Corrected typo here
