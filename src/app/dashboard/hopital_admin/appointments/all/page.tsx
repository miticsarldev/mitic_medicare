...
export default function AppointmentsTable() {
        ...
        if (dateRange?.from && dateRange?.to) {
          filteredAppointments = filteredAppointments.filter(
            (appt: Appointment) => {
              ...
              return (
                appointmentDate >= dateRange.from! &&
                appointmentDate <= dateRange.to!
              );
            }
          );
        }
  ...
  ); // Ajoutez dateRange aux dépendances
                    ...
                    <CardContent className="space-y-2 text-sm">
                      <div className="flex items-center gap-2 truncate">
                        ...
                        <span className="truncate">
                          {appt.doctor.name} (
                          {getSpecializationLabel(appt.doctor.specialization)})
                        </span>
                      </div>
                      ...
                      <div className="flex items-center gap-2">
                        <Badge
                          className={
                            STATUS_COLORS[
                              appt.status as keyof typeof STATUS_COLORS
                            ] || "bg-gray-100"
                          }
                        >
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                          ...
                          <Badge
                            className={
                              STATUS_COLORS[
                                selectedAppointment.status as keyof typeof STATUS_COLORS
                              ] || "bg-gray-100"
                            }
                          >
                      ...
                      <div className="pl-6 space-y-6 text-sm">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          ...
                          <div>
                            ...
                            <p>
                              {format(
                                new Date(
                                  selectedAppointment.medicalRecord.updatedAt
                                ),
                                "PPpp",
                                { locale: fr }
                              )}
                        ...
                        <div className="space-y-3">
                          <p className="font-semibold">
                            Pièces jointes (
                            {
                              selectedAppointment.medicalRecord.attachments
                                .length
                            }
                            )
                          </p>
                          {selectedAppointment.medicalRecord.attachments
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                              {selectedAppointment.medicalRecord.attachments.map(
                                (file) => (
                                  <div
                                    key={file.id}
                            ...
                            <div className="p-4 bg-muted/30 text-sm text-muted-foreground rounded-md">
                              Aucune pièce jointe disponible pour ce
                              rendez-vous.
                            </div>
                          )}
                        </div>
                        ...
                        <div className="space-y-3 pt-4">
                          <p className="font-semibold">
                            Prescriptions (
                            {
                              selectedAppointment.medicalRecord.prescriptions
                                .length
                            }
                            )
                          </p>
                          {selectedAppointment.medicalRecord.prescriptions
                            .length > 0 ? (
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                              {selectedAppointment.medicalRecord.prescriptions.map(
                                (prescription) => (
                                  <div
                                    key={prescription.id}
                                    className="flex items-center gap-3 p-3 border rounded-md hover:bg-muted/50 transition-colors"
                            ...
                            <div className="p-4 bg-muted/30 text-sm text-muted-foreground rounded-md">
                              Aucune prescription enregistrée pour ce
                              rendez-vous.
                            </div>
                          )}
                        </div>
