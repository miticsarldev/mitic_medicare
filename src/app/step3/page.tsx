  ...
  return (
    ...
    <Navbar />
        <div className="p-6 max-w-xl mx-auto bg-white shadow-lg rounded-lg">
          ...
          <p className="text-gray-600 font-semibold">(admin@gmail.com)</p>
          <form onSubmit={handleSubmit} className="mt-4 flex justify-center items-center">
              <input
                  type="text"
                  className="border p-2 w-24 text-center text-lg tracking-widest rounded-l"
                  placeholder="####"
                  value={code}
                  maxLength={4}
                  onChange={(e) => setCode(e.target.value.replace(/\D/g, ""))} // Empêche la saisie de lettres
              />
              <button
                  type="submit"
          ...
          <p className="text-gray-500 text-sm mt-2">Votre connexion est sécurisée</p>
      </div></>
  );
};
