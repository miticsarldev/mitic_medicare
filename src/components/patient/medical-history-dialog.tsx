...
export function MedicalHistoryDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-[calc(100%-1rem)] max-w-[95vw] sm:max-w-2xl max-h-[90vh] p-0">
        <DialogHeader className="p-4 sm:p-6 pb-0">
          <div className="flex flex-col sm:flex-row items-start sm:items-center sm:justify-between gap-3">
            <DialogTitle className="text-lg sm:text-xl pr-2 sm:pr-8">
              {item.title}
            </DialogTitle>
            <Badge
              variant="outline"
              className={`${getStatusColor(item.status)} flex items-center space-x-1 shrink-0`}
            >
        ...
        <ScrollArea className="max-h-[calc(90vh-8rem)] sm:max-h-[calc(90vh-10rem)]">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="text-sm">
                    {formatDateOnly(item.diagnosedDate)}
                  </p>
                </div>
